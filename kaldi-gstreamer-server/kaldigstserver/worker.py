...
from decoderLIUM import DecoderPipelineLIUM
...
class ServerWebsocket(WebSocketClient):
    ...
    def _on_result(self, result, final):
        ...
        try:
            self.send(json.dumps(event))
#            logger.info(" c'est parti : seg %d :%s" % (self.num_segments, processed_transcript) )
        except:
            e = sys.exc_info()[1]
...
def main():
    ...
    global USE_LIUM
    USE_LIUM =conf.get("use-lium",True)
    global SILENCE_TIMEOUT
    ...
    if USE_NNET2:
        ...
        if USE_LIUM:
            decoder_pipeline= DecoderPipelineLIUM(conf)
        else:
