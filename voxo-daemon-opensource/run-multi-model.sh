...
DOCKER_NAME=voxo-daemon-multi-model

nvidia-docker run -dti --name $DOCKER_NAME -v $OUTPUT_DIR:/mnt -v $CONF_FILE:/opt/voxo-daemon/daemon.conf -v /opt/asr/models/:/mnt/models -v /opt/asr/output:/mnt/output -e "DOCKER_HOST=$(ip -4 addr show docker0 | grep -Po 'inet \K[\d.]+'):8000" voxolab/daemon-multi-model /opt/voxo-daemon/decode_daemon_start.sh
